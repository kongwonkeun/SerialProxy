...
int cfg_readstr(const char *filename, cfg_item_s *item, const char *deflt)
{
    char buf[BUFSIZ];
...
int cfg_readint(const char *filename, cfg_item_i *item, int deflt)
{
...
char *cfg_readbuf(const char *filename, const char *key, char *value, size_t valuesiz)
{
    char *ptr;
    int len;
    ...
    while (fgets(value, (int)valuesiz, f)) {
